...
class AddColumnsInTestTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('tests', function (Blueprint $table) {
            ...
            $table->foreign('started_by_user')->references('id')->on('users')->onDelete('cascade');
            $table->integer('finished_by_user')->unsigned()->nullable()->after('finished_at');
            $table->foreign('finished_by_user')->references('id')->on('users')->onDelete('cascade');
            $table->integer('graded_by_user')->unsigned()->nullable()->after('graded_at');
            $table->foreign('graded_by_user')->references('id')->on('users')->onDelete('cascade');
        });
    }
